...
import { LookHorizontalCarousel } from "./LookHorizontalCarousel";
...
export function Lookbook() {
  ...
  const activeLookIdxRef = useRef(0);
  ...
  useEffect(() => {
    activeLookIdxRef.current = activeLookIdx;
  }, [activeLookIdx]);
  ...
  const focusIdx = focus.open ? focus.idx : -1;
  useEffect(() => {
    if (!focus.open) {
  ...
  useLayoutEffect(() => {
    ...
    // Pendant l'exit, on évite tout "fond" forcé pour conserver le rendu.
  ...
  const startExitCoverTransition = (idx: number) => {
    const lookKey = lookbook[idx]?.src as LookbookImageKey;
    const title = lookbook[idx]?.title ?? "";
    const st = gridAnim.current;
    const cardR =
    ...
    // Anime la cover du plein écran vers la carte dans la grille
    // (évite un flash blanc: on garde l'image pendant toute la transition).
    setCoverTransition({
      idx,
      lookKey,
      title,
  ...
  return (
    <section className="w-full">
      ...
      {coverTransition && (
        <div
          className="fixed z-[1001] overflow-hidden bg-transparent"
          style={{
            left:
